...
async def get_dishes(menu_id: int, submenu_id: int) -> list[DishSchemaResponse]:
    service = DishService()
    return service.get_dishes(menu_id, submenu_id)


@dish_router.get('/dishes/{dish_id}')
async def get_dish(menu_id: int, submenu_id: int, dish_id: int) -> DishSchemaResponse:
    service = DishService()
    return service.get_dish(menu_id, submenu_id, dish_id)


@dish_router.post('/dishes', status_code=201)
async def add_dish(menu_id: int, submenu_id: int, dish: DishSchema) -> DishSchemaResponse:
    service = DishService()
    return service.add_dish(menu_id, submenu_id, dish)


@dish_router.patch('/dishes/{dish_id}')
async def edit_dish(menu_id: int, submenu_id: int, dish_id: int, dish: DishSchema) -> DishSchemaResponse:
    service = DishService()
    return service.edit_dish(menu_id, submenu_id, dish_id, dish)


@dish_router.delete('/dishes/{dish_id}')
async def delete_dish(menu_id: int, submenu_id: int, dish_id: int) -> dict:
    service = DishService()
